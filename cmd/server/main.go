...
func main() {
	...
	if err != nil {
		cfg.Logger.Fatalf("error when getting config %v ", err)
	}

	cfg.Logger.Infoln("!!! SERVER START !!!")
	...
	if dbstor {
		...
		if err != nil {
			cfg.Logger.Fatalf("error when making migration %v ", err)
		}
	}
	...
	if cfg.Restore {
		...
		if err != nil {
			cfg.Logger.Fatalf("error when restoring metircs from file %v ", err)
		}
	}
	...
	if err != nil {
		cfg.Logger.Fatalf("error when start server %v ", err)
	}
}
